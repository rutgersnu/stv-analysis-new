...
void cc0pi_slice_plots() {

  #ifdef USE_FAKE_DATA
    // Initialize the FilePropertiesManager and tell it to treat the NuWro
    // MC ntuples as if they were data
    auto& fpm = FilePropertiesManager::Instance();
    fpm.load_file_properties( "cc0pi_nuwro_file_properties.txt" );
  #else
    auto& fpm = FilePropertiesManager::Instance();
    fpm.load_file_properties( "cc0pi_file_properties.txt" );
  #endif

  auto* syst_ptr = new MCC9SystematicsCalculator(
    "/uboone/data/users/mastbaum/cc0pi/univmake_cc0pi_output.root",
    "systcalc.conf" );
  auto& syst = *syst_ptr;
  ...
  auto* sb_ptr = new SliceBinning( "cc0pi_slice_config.txt" );
  auto& sb = *sb_ptr;
