...
//  std::vector< double > nbins = {-1.,-0.775,-0.675,-0.575,-0.475,-0.4,-0.325,-0.25,-0.175,-0.1,-0.025,0.025,0.1,0.175,0.225,0.275,0.325,0.375,0.425,0.475,0.525,0.575,0.625,0.675,0.725,0.775,0.825,0.85,0.875,0.9,0.925,0.95,0.975,1.};
  ...
  std::vector<double> nbins = {600.,810.,910.,1005.,1100.};
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  inv_data_covmat_hist->SetMarkerSize(0.4);
  inv_data_covmat_hist->Draw("colz text");
  c4->SaveAs("testing_mat.pdf"); 
...
TH2D *h_error = new TH2D(*result.unfolding_matrix_);
TCanvas* c40 = new TCanvas("c4");
gStyle->SetPaintTextFormat("4.2f");
h_error->SetMarkerColor(kRed);
h_error->SetMarkerSize(0.2);
  ...
  for ( int t = 0; t < num_true_bins; ++t ) { 
      ...
      if ( t < nbins.size() - 1) {
        ...
        ///////////////////////////////////////////////////
         ...
         std::cout<<"Scaled CV_true_signal to BNB POT: "<<true_signal->operator()( t, 0 )*0.259<<std::endl;
...
//	tTrig->SetBranchAddress("weight_All_UBGenie",&All_weight);
 ...
 for (int i_tree=0; i_tree<tree->GetEntries(); i_tree++) {//Loop over the entries.
  ...
  leg->AddEntry((TObject*)0, TString::Format("#chi^{2} / d.o.f = %.2f / %g", chi_square, dof), "");
